...
{
    if (needle.empty() || has_match(needle, haystack) == 0) { return lowest_score_t; }
    const auto s =  ::match(needle.c_str(), haystack.c_str());
    if (s == SCORE_MIN) { return lowest_score_t; }
    return static_cast<double>(s);
}

auto match_positions(const std::string &needle, const std::string &haystack) -> std::pair<double, std::vector<size_t>> 
{
    if (needle.empty() || has_match(needle, haystack) == 0) { return {lowest_score_t, {}}; }

    std::vector<size_t> positions(needle.size());
    const auto score = ::match_positions(needle.c_str(), haystack.c_str(), positions.data());
    if (score == SCORE_MIN) { return {lowest_score_t, {}}; }
    return {static_cast<double>(score), std::move(positions)};
}

// ============================================================================
// Batch fzy matching functions

auto match(const std::string &needle, const std::vector<std::string> &haystack) -> std::vector<size_t>
{
    ...
    for (size_t i = 0; i < haystack.size(); ++i) {
        auto s = match(needle, haystack[i]);
        if (s > lowest_score_t) { matches.push_back({i, s}); }
    }
...
auto match_positions(const std::string &needle, const std::vector<std::string> &haystack) -> std::vector<std::pair<size_t, std::vector<size_t>>> 
{
    if (needle.empty()) { return {}; }
